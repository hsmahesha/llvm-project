...
static MachineFunction *GetErrorMachineFunction(Function *F,
  ...
  for (MachineBasicBlock &MBB : MF) {
    ...
    if (MBB.pred_empty()) {
      ...
      if (MBB.pred_size()  > 1) {
        ...
        // which is computed in all predecessors block.
...
bool X86CFCSS::runOnModule(Module &M) {
  ...
  MachineFunction *ErrMF = GetErrorMachineFunction(ErrF, MMI);
